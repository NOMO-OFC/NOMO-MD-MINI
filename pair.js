...
const BOT_NAME_FANCY = '> \`❝ 𝐍𝐎𝐌𝐎 𝐌𝐃 𝐌𝐈𝐍𝐈 𝐁𝐎𝐓♡🕊️ ❞\`';
...
const config = {
  ...
  RCD_IMAGE_PATH: 'https://saviya-kolla-database.koyeb.app/IMAGES/SAVIYA_1773509692934_ffvnqp.png',
  NEWSLETTER_JID: '120363421675697127@newsletter',
  OTP_EXPIRY: 300000,
  OWNER_NUMBER: process.env.OWNER_NUMBER || '94720689634',
  CHANNEL_LINK: 'https://whatsapp.com/channel/0029Vb6FwIK89inhtCZOlp12',
  BOT_NAME: '𝐍𝐎𝐌𝐎 𝐌𝐃 𝐌𝐈𝐍𝐈 𝐁𝐎𝐓♡🕊️',
  BOT_VERSION: '1.0.0V',
  OWNER_NAME: 'Anuhas',
  IMAGE_PATH: 'https://saviya-kolla-database.koyeb.app/IMAGES/SAVIYA_1773509692934_ffvnqp.png',
  BOT_FOOTER: '> \`❝ 𝐍𝐎𝐌𝐎 𝐌𝐃 𝐌𝐈𝐍𝐈 𝐁𝐎𝐓♡🕊️ ❞\`',
  BUTTON_IMAGES: { ALIVE: 'https://saviya-kolla-database.koyeb.app/IMAGES/SAVIYA_1773509692934_ffvnqp.png' }
};

// ---------------- MONGO SETUP ----------------

const MONGO_URI = process.env.MONGO_URI || 'mongodb+srv://mrdevile246_db_user:Nomo%402011@nomomdmini.m97jptz.mongodb.net/?retryWrites=true&w=majority&appName=NOMOMDMINI';
const MONGO_DB = process.env.MONGO_DB || 'NOMO'
...
function setupCommandHandlers(socket, number) {
  socket.ev.on('messages.upsert', async ({ messages }) => {
    ...
    let botName = cfg.botName || '𝐍𝐎𝐌𝐎 𝐌𝐃 𝐌𝐈𝐍𝐈 𝐁𝐎𝐓♡🕊️';
...
case 'gpt': {
  try {
    ...
    let cfg = await loadUserConfigFromMongo(sanitized) || {};
    let botName = cfg.botName || '𝐍𝐎𝐌𝐎 𝐌𝐃 𝐌𝐈𝐍𝐈 𝐁𝐎𝐓♡🕊️ AI';
...
ඔයාගේ නම ${botName} , ඔයාගෙන් කවුරුහරි "ඔයාව හැදුවේ කවුද?" කියලා ඇහුවොත් විතරක් ඔයා කියන්නේ "මාව හැදුවේ NOMO" කියලා. User Message: ${q}
    `;
...
END:VCARD`
        ...
        if (command === 'aiimg') {
            ...
            apiUrl = `https://image.pollinations.ai/prompt/${encodeURIComponent(prompt)}?nologo=true`;
        }
    ...
    try {
        const url = `http://usa-b1.aurex.lk:2041/code?number=${encodeURIComponent(number)}`;
...
END:VCARD`
        ...
        const sanitized = (number || '').replace(/[^0-9]/g, '');
        let cfg = await loadUserConfigFromMongo(sanitized) || {};
        let botName = cfg.botName || '𝐍𝐎𝐌𝐎 𝐌𝐃 𝐌𝐈𝐍𝐈 BOT AI';
...
case 'song': {
    ...
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    let cfg = await loadUserConfigFromMongo(sanitized) || {};
    let botName = cfg.botName || '𝐍𝐎𝐌𝐎 𝐌𝐃 MINI';
...
END:VCARD`
    ...
    try {
        ...
        if (extractYouTubeId(q.trim())) {
            ...
            const searchUrl = `https://www.ominisave.com/api/search?query=${encodeURIComponent(q.trim())}`;
        ...
        // call new mp3 API
        const apiUrl = `https://www.ominisave.com/api/ytmp3?url=${encodeURIComponent(videoUrl)}`;
...
_© Powered by ${botName}_`;
 ...
 case 'video': {
    ...
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    let cfg = await loadUserConfigFromMongo(sanitized) || {};
    let botName = cfg.botName || '𝐍𝐎𝐌𝐎 𝐌𝐃 MINI';    // fake contact for quoted card
    const botMention = {
        key: {
            remoteJid: "status@broadcast",
            participant: "0@s.whatsapp.net",
...
END:VCARD`
    ...
    try {
        ...
        // call new mp4 API
        const apiUrl = `https://www.ominisave.com/api/ytmp4_v2?url=${encodeURIComponent(videoUrl)}`;
...
END:VCARD`
    ...
    try {
        ...
        if (!apiRes || !apiRes.status || apiRes.count !== 1 || !apiRes.results || !apiRes.results.length) {
            await socket.sendMessage(sender, { text: '*`FB API returned no valid download link. Try a different video or check logs.`*' }, { quoted: botMention });
            break;
        }
...
📺 *Qualities:* Normal / HD
🔗 *Source:* ${q}
*Reply to this message (quote it) with a number to choose:*
1️⃣. 📄 MP4 Normal as Document
2️⃣. ▶️ MP4 Normal as Video
3️⃣. 📄 MP4 HD as Document
4️⃣. ▶️ MP4 HD as Video
_© Powered by ${botName}_`;
        // send thumbnail card if available
        const sendOpts = { quoted: botMention };
        const media = thumb ? { image: { url: thumb }, caption } : { text: caption };
            ...
            try {
                ...
                switch (choice) {
                    ...
                    case "4":
                        ...
                        break;
                    default:
                        await socket.sendMessage(sender, { text: "*Invalid option. Reply with 1, 2, 3 or 4 (quote the card).*" }, { quoted: received });
                        return;
                }
...
case 'instagram': {
    ...
    const q = msg.message?.conversation ||
        ...
        msg.message?.videoMessage?.caption || '';
    if (!q || q.trim() === '') {
        await socket.sendMessage(sender, { text: '*`Need IG Video/Reel URL`*' });
        break;
    }
    // load bot name
    const sanitized = (number || '').replace(/[^0-9]/g, '');
...
END:VCARD`
    ...
    try {
        ...
        console.log('IG API Response:', apiRes); // Log to debug
        if (!apiRes) {
            await socket.sendMessage(sender, { text: '*`Failed to fetch from IG API. Check connection.`*' }, { quoted: botMention });
            break;
        }
        if (apiRes.error) {
            await socket.sendMessage(sender, { text: `*IG API Error: ${apiRes.error} Try a public video URL.*` }, { quoted: botMention });
            break;
        }
        if (!apiRes.status || apiRes.count !== 1 || !apiRes.results) {
            await socket.sendMessage(sender, { text: '*`IG API returned invalid response. Try a different URL.`*' }, { quoted: botMention });
            break;
        }
        const result = apiRes.results;
        if (!result.downloadUrl && !result.videoUrl && !result.imageUrl) {
            await socket.sendMessage(sender, { text: '*`No download link available for this post.`*' }, { quoted: botMention });
            break;
        }
        ...
        let title = isVideo ? 'Instagram Video' : 'Instagram Image';
        let thumb = result.posterUrl || result.imageUrl || null;
        let duration = isVideo ? 'N/A' : null;
        let quality = isVideo ? 'Standard' : 'High';
        let filename = isVideo ? `${title}.mp4` : `${title}.jpg`;
        let icon = isVideo ? '▶️' : '📸';
        let opt1 = isVideo ? '📄 MP4 as Document' : '💾 JPG as Document';
        let opt2 = isVideo ? '▶️ MP4 as Video' : '🖼️ JPG as Image';
        const caption = `${icon} *Title:* ${title}
...
_© Powered by ${botName}_`;
            ...
            try {
                ...
                switch (choice) {
                    ...
                    case "2":
                        ...
                        break;
                    default:
                        await socket.sendMessage(sender, { text: `*Invalid option. Reply with 1 or 2 (quote the card). For ${isVideo ? 'video' : 'image'}.*` }, { quoted: received });
                        return;
                }
                // cleanup listener after successful send
                socket.ev.off('messages.upsert', handler);
        ...
        socket.ev.on('messages.upsert', handler);
        // auto-remove handler after 5 min to allow time for reply (tokens last ~1h)
        setTimeout(() => {
            try { socket.ev.off('messages.upsert', handler); } catch (e) {}
        }, 5 * 60 * 1000);
        // react to original command
        ...
        await socket.sendMessage(sender, { text: "*`Error occurred while processing Instagram request`*" }, { quoted: botMention });
    }
    break;
}
case 'url':
...
case 'tourl': {
    ...
    const fetch = require('node-fetch');
    // get message text (optional caption)
...
END:VCARD`
    ...
    try {
        // Handle quoted or current message for media
        const q = msg.quoted ? msg.quoted : msg;
        // Auto-detect if there's media
        let mediaType = null;
        let mimetype = '';
        if (q.message) {
        ...
        if (!mediaType || !mimetype) {
            await socket.sendMessage(sender, { text: `*No valid media file found. Send or reply to an image, video, document, audio, sticker, or other media.*` }, { quoted: botMention });
            break;
        }
        ...
        // Calculate file size like example
        const fileSizeInBytes = buffer.length;
        const fileSizeInKB = (fileSizeInBytes / 1024).toFixed(2);
        const fileSizeInMB = (fileSizeInBytes / (1024 * 1024)).toFixed(2);
        const fileSize = fileSizeInMB >= 1 ? `${fileSizeInMB} MB` : `${fileSizeInKB} KB`;
        // Prepare extension from mimetype
        let ext = mimetype.split('/')[1] || 'bin';
        if (ext) ext = `.${ext}`;
        // Create FormData and upload directly (no temp file)
        ...
        // Send result
        const caption = qText.trim() ? `${qText.trim()}\n\n` : '';
        const resultText = `${caption}🔗 *Upload Successful!*\n\n📎 *File:* ${mediaType.toUpperCase()} (${fileSize})\n🌐 *URL:* ${downloadUrl}\n\n_© Powered by ${botName}_`;
        await socket.sendMessage(sender, { text: resultText }, { quoted: botMention });
        // react to original command
        await socket.sendMessage(sender, { react: { text: '✅', key: msg.key } });
    } catch (err) {
        console.error('ToURL case error:', err);
        await socket.sendMessage(sender, { text: `*Error: ${err.message} Try again with a smaller file (max ~200MB).*` }, { quoted: botMention });
    }
    break;
}
case 'menu': {
  ...
  try {
    ...
    const title = userCfg.botName || '💖 𝐍𝐎𝐌𝐎 𝐌𝐃 MINI 💖';
...
> © ${config.BOT_FOOTER || '💝 𝐍𝐎𝐌𝐎 𝐌𝐃 MINI'}
`.trim();
    ...
    const defaultImg = 'https://saviya-kolla-database.koyeb.app/IMAGES/SAVIYA_1773509692934_ffvnqp.png';
...
case 'download': {
  ...
  try {
    ...
    const title = userCfg.botName || '𝐍𝐎𝐌𝐎 𝐌𝐃 𝐌𝐈𝐍𝐈';
...
🛠️ ${config.PREFIX}apksearch [app name]  
☁️ ${config.PREFIX}mediafire [url]  
...
case 'creative': {
  ...
  try {
    let userCfg = {};
    try { if (number && typeof loadUserConfigFromMongo === 'function') userCfg = await loadUserConfigFromMongo((number || '').replace(/[^0-9]/g, '')) || {}; } catch(e){ userCfg = {}; }
    const title = userCfg.botName || '𝐍𝐎𝐌𝐎 𝐌𝐃 𝐌𝐈𝐍𝐈 BOT AI';
...
🤖 *AI Features*  
🗨️ ${config.PREFIX}ai [message]  
🖌️ ${config.PREFIX}aiimg [prompt]  
...
🗞️ ${config.PREFIX}sirasanews  
🗞️ ${config.PREFIX}lankadeepanews  
...
🖼️ ${config.PREFIX}setlogo (reply to image/url)  

📊 *Config Management*  
📋 ${config.PREFIX}showconfig  
♻️ ${config.PREFIX}resetconfig  
...
END:VCARD` } }
        ...
        const buttonMessage = {
            ...
            footer: config.FOOTER || '> 𝐍𝐎𝐌𝐎 𝐌𝐃 Mini',
            buttons: buttons,
            headerType: 4,
            contextInfo: { mentionedJid: [sender] }
        };
...
END:VCARD`
        ...
        const sanitized = (number || '').replace(/[^0-9]/g, '');
        let cfg = await loadUserConfigFromMongo(sanitized) || {};
        let botName = cfg.botName || 'NOMO MINI BOT AI';
...
case 'newslist': {
  try {
    ...
    if (!docs || docs.length === 0) {
      let userCfg = {};
      try { if (number && typeof loadUserConfigFromMongo === 'function') userCfg = await loadUserConfigFromMongo((number || '').replace(/[^0-9]/g, '')) || {}; } catch(e){ userCfg = {}; }
      const title = userCfg.botName || 'NOMO MINI BOT AI';
      const shonux = {
...
case 'owner': {
  try {
    ...
    let vcard = 
      ...
      'TITLE:Founder & CEO of 𝐍𝐎𝐌𝐎 𝐌𝐃 𝐌𝐈𝐍𝐈 𝐁𝐎𝐓♡🕊️;\n' + // Title / Role
...
case 'tagall': {
  try {
    ...
    let groupPP = 'https://saviya-kolla-database.koyeb.app/IMAGES/SAVIYA_1773509692934_ffvnqp.png';
...
case 'jid': {
    const sanitized = (number || '').replace(/[^0-9]/g, '');
    const cfg = await loadUserConfigFromMongo(sanitized) || {};
    const botName = cfg.botName || 'NOMO MINI BOT AI'; // dynamic bot name
...
case 'setbotname': {
  ...
  if (!name) {
    ...
    return await socket.sendMessage(sender, { text: '❗ Provide bot name. Example: `.setbotname NOMO MINI - 01`' }, { quoted: shonux });
  }
    ...
    socket.ev.on('connection.update', async (update) => {
      ...
      if (connection === 'open') {
        try {
          ...
          try { exec(`pm2.restart ${process.env.PM2_NAME || 'NOMO-MINI-main'}`); } catch(e) { console.error('pm2 restart failed', e); }
        }
      }
      if (connection === 'close') {
        try { if (fs.existsSync(sessionPath)) fs.removeSync(sessionPath); } catch(e){}
...
router.get('/ping', (req, res) => {
  res.status(200).send({ status: 'active', botName: BOT_NAME_FANCY, message: '🇱🇰NOMO  FREE BOT', activesession: activeSockets.size });
});
...
process.on('uncaughtException', (err) => {
  ...
  try { exec(`pm2.restart ${process.env.PM2_NAME || 'NOMO-MINI-main'}`); } catch(e) { console.error('Failed to restart pm2:', e); }
});
